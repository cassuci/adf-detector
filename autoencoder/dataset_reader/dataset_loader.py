...
class Dataset_ASVspoof(Dataset):
    ...
    def load_spectrogram(self, utt_id):
        if utt_id.endswith('.wav'): # itw case
            spectrogram_scaled = np.load(self.base_dir + utt_id.replace('.wav', '') + ".npy")
        else:    
    ...
    def create_and_save_noise_spectrogram(self, utt_id, wave):
        ...
        if utt_id.endswith('.wav'): # itw case
            np.save(self.base_dir + utt_id.replace('.wav', '') + "_noise.npy", spectrogram_scaled)
        else:
